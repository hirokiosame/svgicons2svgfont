...
function applyTransforms(d, parents) {
	...
	parents.forEach(function(parent) {
		if (undefined !== parent.attributes.transform) {
...
function SVGIcons2SVGFontStream(options) {
	...
	this._transform = function _svgIcons2SVGFontStreamTransform(
		...
		if (glyph.unicode && glyph.unicode instanceof Array && glyph.unicode.length) {
			if (glyph.unicode.some(function(unicodeA, i) {
				...
				_this.emit('error', new Error('Given codepoints for the glyph "' + glyph.name + '" contain duplicates.'));
			}
		} else if ('string' !== typeof glyph.unicode) {
			_this.emit('error', new Error('Please provide a codepoint for the glyph "' + glyph.name + '"'));
		}

		if (glyphs.some(function(anotherGlyph) {
			return (anotherGlyph !== glyph && anotherGlyph.unicode === glyph.unicode);
		})) {
			_this.emit('error', new Error('The glyph "' + glyph.name + '" codepoint seems to be used already elsewhere.'));
		}
			...
			try {
				...
				if ('svg' === tag.name) {
					...
					if (!glyph.width || !glyph.height) {
						log('Glyph "' + glyph.name + '" has no size attribute on which to get the gylph dimensions (heigh and width or viewBox attributes)');
						glyph.width = 150;
						glyph.height = 150;
					}
	...
	this._flush = function _svgIcons2SVGFontStreamFlush(svgFontFlushCallback) {
		...
		options.ascent = undefined !== options.ascent ?
			options.ascent :
...
'<defs>' +
	'<font id="' + options.fontId + '" horiz-adv-x="' + fontWidth + '">' +
		...
		Object.keys(glyphSet).forEach(function(character) {
			if (character.length === 1 && !glyphSet[character]) {
